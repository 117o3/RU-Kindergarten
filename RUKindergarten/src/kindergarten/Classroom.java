...
public class Classroom {
    ...
    public void enterClassroom ( String filename ) {
       ...
       // SNode head = null; // need? if empty?
        ...
        for (int i=0; i<x; i++){
            ...
            // insert student at FRONT of linked list
            SNode newNode = new SNode(info, studentsInLine);
    ...
    public void setupSeats(String seatingChart) {
        ...
        for (int i=0; i< r; i++){
            for (int j=0; j<c; j++){
                seatingLocation[i][j] = StdIn.readBoolean(); //read boolean reads next true/false value on opened file
            }
        }
            ...
            for (int j=0; j<studentsSitting[0].length; j++){
                ...
                if (studentsSitting[i][j] != null){
                    ...
                    if (musicalChairs == null) {
                        musicalChairs = new SNode (studentsSitting[i][j], null); // cant make it next because it doesnt have next until intialized
                        musicalChairs.setNext(musicalChairs);

                        // student can only be at ONE place; delete
                        studentsSitting[i][j] = null; 
                    } else {
                        next = new SNode(studentsSitting[i][j], musicalChairs.getNext());
                        musicalChairs.setNext(next);
                        // move last (musicalChairs) to next; next will be last
                        musicalChairs = next; 
    ...
    public void moveStudentFromChairsToLine(int size) {
        ...
        // WRITE YOUR CODE HERE
        int n = StdRandom.uniform(size); // [0, size)
        int count = 0;

        // musicalChairs points to last of list
        SNode ogLast = musicalChairs;

        // delete front
        if (n == 0){
            // insert into studentsInLine (front= musicalChairs.next)
            insertByHeight(musicalChairs.getNext().getStudent());

            // delete: last points to front.next
            musicalChairs.setNext(musicalChairs.getNext().getNext());

        // delete last
        } else if (n == size -1){ // [0,size)
            // iterates until musicalChairs = 2nd to last
            while (count != n){
                musicalChairs = musicalChairs.getNext();
                count++;
            }
            // insert into studentsInLine (last = musicalChairs.next)
            insertByHeight(musicalChairs.getNext().getStudent());
            musicalChairs.setNext(musicalChairs.getNext().getNext());
            // musicalChairs still referring to last
        
        // delete middle
        } else {
            // iterates until musicalChairs = prev of n || n-1
            while (count != n){
                musicalChairs = musicalChairs.getNext();
                count++;
            }
            // insert into studentsInLine (musicalChairs.next = n)
            insertByHeight(musicalChairs.getNext().getStudent());
            musicalChairs.setNext(musicalChairs.getNext().getNext());
            // musicalChairs always refers to last
            musicalChairs = ogLast;
        }
    }
    ...
    public void insertByHeight(Student studentToInsert) {
        ...
        // if studentToInsert is shorter than front of SIL
        } else if (studentToInsert.getHeight() < studentsInLine.getStudent().getHeight()){            
            insert.setNext(studentsInLine);
            // update front
            studentsInLine = insert;
        
        // if studentToInsert is taller than front
        } else {
            // transverse through line if insert is taller or equal to current node (cmp.next, because cmp is SIL; already checked previously)
            // tallest when cmp.next = null; finish transversing
            cmp = studentsInLine; 
            while (cmp.getNext() != null && cmp.getNext().getStudent().getHeight() <= studentToInsert.getHeight()) {
                cmp = cmp.getNext();
            }
            // not taller than cmp.next, but taller/equal to cmp
            // update
            insert.setNext(cmp.getNext());
            cmp.setNext(insert);
        }
    ...
    public void eliminateLosingStudents() {
        ...
        // WRITE YOUR CODE HERE
        // starts at 1 because starts at front of CLL, and ends at end (excluding)
        int size = 1;
        // start at front of linked list
        SNode count = musicalChairs.getNext();

        // stop when reach front (again)
        while (count != musicalChairs){
            size ++;
            count = count.getNext();
        }

        while (size != 1){ // leave ONE in musicalChairs
    ...
    public void seatMusicalChairsWinner () {
        ...
        // WRITE YOUR CODE HERE
        // same as previous: counting size of musicalChairs
        int size = 1;
        SNode count = musicalChairs.getNext();
        while (count != musicalChairs){
            size ++;
        ...
        // ONLY IF MUSICAL CHAIRS IS SIZE 1
        if (size == 1){
            for (int i=0; i<seatingLocation.length; i++){
                for (int j=0; j<seatingLocation[0].length; j++){
